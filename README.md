...
- [Overview](#overview)
- [Data Sources and Dataset Scope](#data-sources-and-dataset-scope)
...
## Data Sources and Dataset Scope

This repository contains platform code and reproducible demo workflows. It does **not** include proprietary production retail datasets.

Data paths used in this repo:

- **Synthetic (default):** deterministic transactions generated by `ingestion/generator/generate.py`.
- **Real-data option:** UCI Online Retail II CSV, normalized to canonical transaction shape by `ingestion/real/uci_online_retail.py`.
- **Demo screenshots:** generated from a local run (`scripts/generate_readme_demos.py`) using synthetic source data and temporary Gold outputs.

Canonical transaction fields expected by ingestion and Spark batch ETL:

- `transaction_id`, `ts_utc`, `store_id`, `customer_id`, `product_id`
- `quantity`, `unit_price`, `currency`, `payment_method`, `channel`, `promo_id`

Produced analytical datasets:

- `data/lakehouse/bronze/transactions`
- `data/lakehouse/silver/transactions`
- `data/lakehouse/gold/daily_revenue_by_store`
- `data/lakehouse/gold/top_10_products_by_day`
- `data/lakehouse/gold/customer_lifetime_value`
